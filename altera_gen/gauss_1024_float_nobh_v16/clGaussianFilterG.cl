...
float clGaussianFilterGKernelKernel(float Input[5][5]) {
    {
        ...
        {
            _tmp0 += 1458 * getWindowAt(Input, 0, 2 + -1);
        }
        {
...
__kernel __attribute__((reqd_work_group_size(1, 1, 1)))
 void clGaussianFilterGKernel(__global float * restrict OUT, __global const float * restrict IN) {
    process(16, float, float, OUT, ARRY, IN, ARRY, HIPACC_MAX_WIDTH, HIPACC_MAX_HEIGHT, clGaussianFilterGKernelKernel, 5, 5, UNDEFINED);
}
